...
graph TB
    ...
    RPC["RPC Connection<br/>(Solana Devnet)"]
    
    subgraph "Blockchain"
        SC["Smart Contract<br/>(Anchor/Rust)<br/>Escrow PDA<br/>State Accts"]
        Backend["Backend API<br/>(Optional)<br/>Metadata<br/>Off-chain IPFS"]
        Token["Token Program<br/>(SPL Tokens)<br/>USDC/SOL<br/>Vault PDA"]
    end
    ...
    style RPC fill:#e1f5fe,stroke:#01579b
    style SC fill:#fff3e0,stroke:#e65100
    style Backend fill:#e8f5e8,stroke:#2e7d32
    style Token fill:#f3e5f5,stroke:#6a1b9a
```
