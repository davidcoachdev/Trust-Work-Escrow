...
sequenceDiagram
    ...
    participant A as Árbitro

    Note over Client: init (solo 1 vez, admin)
    Client->>F: create + deposit
    F->>Client: accept
    F->>Client: submit
    Client->>F: approve (libera fondos)
    
    Note over Client,F: O en caso de disputa
    Client->>F: reject (o)
    F->>A: raise-dispute
    A->>F: resolve-dispute (árbolo decide %)
```
