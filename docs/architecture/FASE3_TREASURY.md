...
```mermaid
flowchart LR
    subgraph "1. Cliente Publica"
        Job["Job: App Web<br/>Monto: 5 USDC<br/>Comisión: 5% → 0.25 USDC"]
    end
    
    subgraph "2. Cliente Deposita"
        CW["Cliente Wallet<br/>-5 USDC"] --> AW["Wallet App<br/>+5 USDC<br/>Estado: OK"]
    end
    
    subgraph "3. Freelancer Trabaja"
        Accept["Acepta"] --> Progress["Job: InProgress<br/>Entregado"]
    end
    
    subgraph "4. App Verifica y Paga"
        Approve["Cliente<br/>Aprueba"] --> Pay["Wallet App<br/>-4.75 USDC<br/>-0.50 USDC fee"]
        Pay --> Freelancer["Freelancer<br/>+4.75 USDC<br/>(95% neto)"]
    end
    ...
    style Job fill:#e1f5fe,stroke:#01579b
    style AW fill:#fff3e0,stroke:#e65100
    style Pay fill:#f3e5f5,stroke:#6a1b9a
    style Freelancer fill:#e8f5e8,stroke:#2e7d32
```
...
graph TB
    subgraph "Frontend"
        Client["Cliente<br/>(On-chain)"] 
        Freelancer["Freelancer<br/>(On-chain)"]
    end
    
    subgraph "App (Off-chain)"
        App["App<br/>(Off-chain)"]
    end
    
    subgraph "Smart Contract (On-chain)"
        Job["Job<br/>(State)"]
        Config["Config<br/>(Settings)"]
        Events["Events<br/>(On-chain)"]
    end
    
    Client -->|"create_job"| App
    App -->|"1. create_job"| Job
    Client -->|"2. deposita"| App
    App -->|"verifica"| App
    App -->|"3. accept/submit"| Job
    Freelancer -->|"accept/submit"| App
    App -->|"4. approve"| Job
    App -->|"paga"| Freelancer
    App -->|"fee"| App
    
    Client -.->|"verificación"| Config
    App -.->|"config"| Config
    Job -.->|"emite"| Events
    
    style Client fill:#e1f5fe,stroke:#01579b
    style Freelancer fill:#e1f5fe,stroke:#01579b
    style App fill:#fff3e0,stroke:#e65100
    style Job fill:#e8f5e8,stroke:#2e7d32
    style Config fill:#e8f5e8,stroke:#2e7d32
    style Events fill:#f3e5f5,stroke:#6a1b9a
```
