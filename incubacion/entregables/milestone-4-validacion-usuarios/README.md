# 🔍 Milestone 4 — Validación Inicial con Usuarios

**Fecha de entrega:** Viernes 31 de Julio

**Documento completo:** [`trust-escrow-milestone-4-plan-validacion.md`](trust-escrow-milestone-4-plan-validacion.md)

---

### ✅ Checklist de preparación

- [x] Hipótesis principal y 5 sub-hipótesis definidas
- [x] 3 perfiles de usuario (freelancer, cliente, árbitro)
- [x] Guía de entrevistas 1:1 (2 sets de preguntas)
- [x] Encuesta online de validación (4 secciones, 10 preguntas)
- [x] Guión de testing de prototipo
- [x] KPIs y métricas con metas
- [x] Canales de reclutamiento identificados
- [x] Cronograma semana a semana

### 🔲 Pendiente para las próximas 3 semanas

| Semana | Actividad | Estado |
|--------|-----------|--------|
| 14-18 Jul | Realizar 5-8 entrevistas 1:1 | 🔲 |
| 21-25 Jul | Lanzar encuesta online + analizar resultados | 🔲 |
| 28-30 Jul | Testing de prototipo con 3-5 usuarios | 🔲 |
| 31 Jul | Compilar entregable final | 🔲 |

### 📂 Archivos en este entregable

| Archivo | Descripción |
|---------|-------------|
| `README.md` | Este índice + checklist |
| `trust-escrow-milestone-4-plan-validacion.md` | Plan completo de validación con templates para registro |
| [`herramientas/README.md`](herramientas/README.md) | 🧰 Índice de las herramientas listas-para-usar (ver abajo) |

### 🧰 Carpeta `herramientas/` (materiales ejecutables)

Derivados del feedback de Business Foundation (priorizar segmento, jerarquizar factores de decisión, documentar confianza de migración).

| # | Archivo | Qué aporta |
|---|---------|------------|
| 01 | [`herramientas/preguntas-entrevista.md`](herramientas/01-preguntas-entrevista.md) | Guion de entrevistas 1:1 (freelancer/cliente/árbitro) |
| 02 | [`herramientas/mercado-objetivo.md`](herramientas/02-mercado-objetivo.md) | Segmento prioritario (freelancer crypto-native) + TAM/SAM/SOM |
| 03 | [`herramientas/recomendaciones-trabajo.md`](herramientas/03-recomendaciones-trabajo.md) | Canales de entrevista + cómo sacar la **ponderación** de respuestas |
| 04 | [`herramientas/encuesta-online.md`](herramientas/04-encuesta-online.md) | Encuesta cerrada (Typeform/Forms, 12 preg, 3-4 min) |
| 05 | [`herramientas/mensajes-difusion.md`](herramientas/05-mensajes-difusion.md) | Borradores de difusión (Reddit/Telegram/LinkedIn/X/Discord) |
| 06 | [`herramientas/mensajes-x-linkedin-discord.md`](herramientas/06-mensajes-x-linkedin-discord.md) | Versiones cortas listas-para-pegar en X, LinkedIn y Discord |

> 💡 Flujo sugerido: leer `02` → difundir con `05`/`06` → entrevistar con `01` → lanzar encuesta `04` → ponderar con `03` → volcar resultados al plan principal.

### 🔗 Referencias

- [Guía User Research →](../referencias/10-user-research-guia.md)
- [Guía Testing de MVP →](../referencias/13-testing-mvp-guia.md)
- [Guía Validación de Idea →](../referencias/09-validacion-idea-guia.md)
- [Guía Design Thinking →](../referencias/08-design-thinking-guia.md)
- [Requisitos del programa →](../referencias/programa-milestones.md)
- [Milestone 1 — Usuario objetivo →](../../entregables/milestone-1-roadmap/trust-escrow-milestone-1-roadmap.md)
